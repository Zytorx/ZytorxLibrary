...
public class SimpleBlockCollection implements BlockCollection {
    ...
    public SimpleBlockCollection(Registrar registrar, String name, Supplier<Block> block, CreativeModeTab tab) {
        var nameWithoutSuffix = getNameWithoutSuffix(name);


        standardBlock = registrar.createBlock(nameWithoutSuffix + STANDARD_SUFFIX, block, tab);
